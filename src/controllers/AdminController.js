...
export const AdminLogin = asyncWrapper(async (req, res, next) => {
  ...
  if (admin.roleLevel != 0) {
    const error = appError.create(
      "ليس لديك صلاحية الدخول كمسؤول",
      403,
      httpStatusText.FAIL
    ); // 403 Forbidden أنسب للصلاحيات
    return next(error);
  }
...
export const AddAdmin = asyncWrapper(async (req, res, next) => {
  ...
  if (!req.userId) {
    const error = appError.create(
      "غير مصرح لك بإتمام هذه العملية",
      401,
      httpStatusText.FAIL
    );
    return next(error);
...
export const deleteUser = asyncWrapper(async (req, res, next) => {
  ...
  if (!deletedUser) {
    return next(
      appError.create("المستخدم غير موجود", 404, httpStatusText.FAIL)
    );
  }
...
export const AddRole = asyncWrapper(async (req, res, next) => {
  ...
  const newAdmin = await Admin.addUserRoleData(userId, roleId, departmentId);

  if (!newAdmin) {
    const error = appError.create(
      "هذا الدور غير متاح لهذا القسم",
      404,
      httpStatusText.FAIL
    );
    return next(error);
